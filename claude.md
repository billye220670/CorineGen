...
- **特殊字符（可选）**: `[]` 或 `【】` 指定剧情发展走向
- **输入**: 当前分镜的提示词
- **示例**: `少女站在森林边缘，[她发现了一只受伤的小鹿]`
- **输出**: 连贯的下一个分镜提示词
- **特点**: 保持人物、服装、场景的一致性

**4. 生成剧本 (script)** - 生成完整分镜剧本
- **输入**: 故事大纲或情节描述
- **示例**: `一个少女在森林中迷路，遇到了一只会说话的狐狸，狐狸带她找到了回家的路。请生成 4 个分镜。`
- **输出**: 4-8 个完整分镜提示词
- **特点**: 起承转合，全局一致性
...
- 顶部: 标题 + Tab 切换栏（4 个模式）
- 主体左右分栏:
  - **左侧 (45%)**: 提示词输入框、特殊字符指南、生成按钮、错误提示
  - **右侧 (55%)**: 结果列表（可滚动）+ 底部操作按钮（固定）

**特殊字符指南**:
- 所有 4 种模式都有对应的中文使用提示
- variation: `少女，#穿着红色连衣裙@0.8(希望蓝色调)，站在花园里`
- polish: `少女，[穿着裙子......]，站在[花园..]里`
- continue: `少女站在森林边缘，[她发现了一只受伤的小鹿]`
- script: `一个少女在森林中迷路，遇到了一只会说话的狐狸，狐狸带她找到了回家的路。请生成 4 个分镜。`

**模式特定交互**:
1. **variation 模式**: 多个变体，每个右侧有 + 按钮添加到主界面
2. **polish 模式**: 单个结果，右侧有 + 按钮添加到主界面
3. **continue 模式**: 单个结果，右侧有 + 按钮添加到主界面
4. **script 模式**: 多个分镜，每个右侧有 + 按钮，底部按钮为"下载分镜脚本"

**+ 按钮功能**:
- 点击后将提示词添加到主界面的提示词列表
- 样式统一使用渐变按钮风格（与"生成"按钮一致）

**模式独立结果存储**:
- 每个模式独立保存生成结果
- 切换模式不会丢失之前的结果
- 数据结构: `{ variation: [], polish: [], continue: [], script: [] }`

#### 状态持久化
- 使用 localStorage 保存面板状态
- 包括: 当前模式、输入内容、生成结果（4 个模式独立）、选中索引
- 关闭面板不清空状态，下次打开恢复
- 滚动条完全隐藏（保持滚动功能）
