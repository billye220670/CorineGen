...
* **设置参数**：通过`response_format`参数指定您定义的 JSON Schema。
* **提示词指引**：在提示词中指引模型进行结构化输出。

## 使用示例

下文提供了完整的 Python 代码示例，演示如何使用 Structured Outputs 功能生成符合您提供的 JSON Schema 的 JSON 响应。

### 1. 初始化客户端

您需要使用您的 JieKou AI API 密钥初始化客户端。
...
您需要定义 JSON Schema。以下示例创建了一个从用户输入中提取费用信息的 JSON Schema。
...
创建 API 请求。此请求包括 `response_format` 参数，指定了上一步中定义的 JSON schema。
