...
**测试结果:**
```
variation 模式: [成功/失败] - 备注: _________
polish 模式: [成功/失败] - 备注: _________
continue 模式: [成功/失败] - 备注: _________
script 模式: [成功/失败] - 备注: _________
```
...
- [ ] 实现状态管理
  - [ ] 默认状态: "生成"
  - [ ] 加载状态: "生成中..." + 加载图标
  - [ ] 禁用状态: `disabled={!input || isGenerating}`
- [ ] 实现验证逻辑
  - [ ] 输入框不能为空
  - [ ] variation/polish 模式下输入为空显示指南
- [ ] 绑定点击事件 `handleGenerate()`
- [ ] 添加样式
