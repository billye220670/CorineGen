...
1. 修改 `frontend/src/App.jsx` 中的 `APP_VERSION` 常量
2. 在版本号后添加注释说明（≤10字）
3. 更新此文档的版本变更日志表格
4. 提交时在 commit message 中注明版本
...
- [x] **第二步**: 修复测试中发现的问题 ✅
  - [x] 版本号布局问题（显示在标题右侧）
  - [x] 飞行模式断开没有暂停提示（添加 WebSocket onclose 监听器）
- [ ] **第三步**: 在手机端充分测试修复后的功能
- [ ] **第四步**: 根据测试结果决定是否实施 P1
- [ ] **第五步**: 根据用户反馈决定是否实施 P2
...
**文档创建时间**: 2026-01-18
**实施状态**: P0 已完成，Bug 修复已完成，等待测试

---

## Bug 修复记录

### Bug #1: 版本号布局打乱页面 (已修复 - 2026-01-18)
...
### Bug #2: 飞行模式断开没有暂停提示 (已修复 - 2026-01-18)

**问题描述**:
- 手机端在生成进度 88% 时打开飞行模式
- 没有任何暂停提示
- 关闭飞行模式后，进度卡在 88%，直接执行下一张

**根本原因**:
- WebSocket 连接建立后，没有监听 `onclose` 事件
- 当飞行模式断开 WebSocket 时，只触发了 `onclose`，但代码没有处理
- 导致循环继续执行，但连接已断开

**修复方案**:
- 在 `generateLoop` 和 `generateBatch` 中添加 `ws.onclose` 监听器
- 检测异常关闭码（`!== 1000` 且 `!== 1005`）
- 异常关闭时：
  - 循环模式：reject Promise，触发 catch 块，调用 `pauseGeneration()`
  - 批次模式：直接调用 `pauseGeneration()`

**修改文件**:
- `frontend/src/App.jsx` (行 1553-1567, 1304-1312)

**修复后行为**:
- 飞行模式断开 → WebSocket 触发 `onclose`
- 检测到异常关闭 → 抛出错误或调用 `pauseGeneration()`
- 弹出"生成已暂停"对话框
- 显示暂停原因："WebSocket连接断开"
- 用户关闭飞行模式后，点击"继续生成"恢复

**测试要求**:
- 手机端打开 Vercel app
- 发起 batch=5 的生成
- 在第 2 张生成进度 50-90% 时打开飞行模式
- 预期：弹出"生成已暂停"对话框
- 关闭飞行模式，点击"继续生成"
- 预期：从第 3 张开始继续生成
...
### 测试环境
- **桌面端**: http://localhost:5173 (开发环境)
- **手机端**: https://corine-gen.vercel.app (生产环境)
...
- [ ] PC 端：标题右侧显示 `v1.1.2`
- [ ] 手机端：标题右侧显示 `v1.1.2`
- [ ] PC 端布局正常：左侧配置面板 + 右侧生图面板

#### ✅ 测试 2: 智能横幅显示
- [ ] 发起 batch=5 的生成
- [ ] 生成第 2 张时，停止后端（`pm2 stop corinegen-backend`）
- [ ] **预期**: 不显示红色"连接断开"横幅
- [ ] **预期**: 弹出"生成已暂停"对话框
- [ ] **新增测试**: 点击横幅"重新连接"（后端未上线）
- [ ] **预期**: 3秒后横幅自动隐藏
- [ ] 重启后端（`pm2 start corinegen-backend`）
- [ ] **新增测试**: 点击横幅"重新连接"（后端已上线）
- [ ] **预期**: 暂停对话框关闭
- [ ] 点击"继续生成"
- [ ] **预期**: 从第 3 张开始继续生成（不卡在 loading）
...
### Bug #3: 重连和恢复时卡住 (已修复 - 2026-01-18)

**问题描述**:
- **问题1**: 后端下线时点击"重新连接"，如果后端未上线，会卡在"正在检查连接..."
- **问题2**: 后端下线后不点击横幅，等后端上线后再点击"继续生成"：
  - 暂停对话框关闭
  - 但断开前正在生成的图片卡在 loading 状态
  - 队列不会继续

**根本原因**:
- **问题1**: 连接检查失败后，横幅一直显示"检查中"状态，没有自动恢复
- **问题2**: `pauseGeneration()` 没有重置占位符的 `isLoading` 和 `progress` 状态
  - 断开时占位符可能有 `isLoading: true`（模型加载中）
  - 恢复时这些状态未清除，导致卡在 loading

**修复方案**:
- **问题1**: 连接检查失败后，3秒自动隐藏横幅
- **问题2**: 暂停时重置占位符状态：
  ```javascript
  {
    ...p,
...
**修改文件**:
- `frontend/src/App.jsx` (行 698-700, 1097-1102)

**修复后行为**:
- 点击重连失败 → 3秒后横幅自动隐藏
- 继续生成 → 占位符从干净的 `queue` 状态重新开始
- 不会卡在 loading 或旧的进度上
